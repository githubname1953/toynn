...
public:
    ...
    {   
        ...
        v[size-1].nout.noalias() = v[size-1].weight*v[size-2].nout + v[size-1].bias;
    ...
    void train(Eigen::MatrixXf &input, Eigen::MatrixXf &target, float lr)
    {
        ...
        v[size-1].dweight.noalias() = lr*(v[size-1].error*v[size-2].nout.transpose());
        v[size-1].weight += v[size-1].dweight;
        v[size-1].bias += lr*v[size-1].error;
